...
package com.doomonafireball.betterpickers.recurrencepicker.zh;
...
public class RecurrencePickerDialog extends DialogFragment implements OnItemSelectedListener,
    ...
    static private void copyEventRecurrenceToModel(final EventRecurrence er,
        ...
        // End by date:
        // ZH: until is epoch second.
        if (er.until != null) {
            if (model.endDate == null) {
                model.endDate = new Time();
                model.endDate.set(TimeUnit.SECONDS.toMillis(er.until));
            }
    ...
    static private void copyModelToEventRecurrence(final RecurrenceModel model,
        ...
        switch (model.end) {
            case RecurrenceModel.END_BY_DATE:
                if (model.endDate != null) {
                    // ZH: Clear endDate to start of day.
                    model.endDate.hour = 0;
                    model.endDate.minute = 0;
                    model.endDate.second = 0;
                    model.endDate.allDay = false;
                    model.endDate.switchTimezone(Time.TIMEZONE_UTC);
                    model.endDate.normalize(false);
                    // ZH: until is epoch second.
                    er.until = TimeUnit.MILLISECONDS.toSeconds(model.endDate.toMillis(false));
                    er.count = 0;
                } else {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mView = inflater.inflate(R.layout.recurrencepicker_zh, container, true);
